...
'''
lines是一个句子用相对于word_dic的索引来表示
state_seq是lines所对应的分词结果
'''
def precess_data():
    ifp = open("RenMinData.txt_utf8", 'r', encoding='utf-8', errors='ignore')
    line_num = 0
    word_dic = {}
    word_ind = 0
    line_seq = []
    ...
    for line in ifp:
        ...
        for i in range(len(line)):
            ...
            # 建立单词表
            if line[i] not in word_dic.keys():
...
wordlists, taglists = prepareData('语料\\train.char.bmes', wordDict, tagDict)

#X,Z,word_dic = precess_data()
wordseg_hmm = hmm.DiscreteHMM(len(tagDict),len(wordDict),1)
wordseg_hmm.train_batch(wordlists, taglists)

print ("startprob_prior: ", wordseg_hmm.start_prob)
print ("transmit: ", wordseg_hmm.transmat_prob)

print (wordseg_hmm.decode(wordlists[0], wordDict))
# sentence_1 = "我要回家吃饭"
